...
def arm_publisher():
    global message_published
    if not message_published:  # 메시지가 아직 발행되지 않았다면
        pub = rospy.Publisher('/order', String, queue_size=6)
        time.sleep(1)  # 1초 기다림
        pub.publish("go")  # 메시지 발행
        message_published = True  # 메시지가 발행되었음을 나타내기 위해 True로 설정
...
if __name__ == '__main__':
    try:
        rospy.init_node('book_check', anonymous=True)
        checkbook()
